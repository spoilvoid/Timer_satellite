...
if __name__ == "__main__":
    ...
    # 从配置中获取参数
    data_tuple_path = os.path.join(args.param_dir, "data_tuple.json") 
    ...
    for satellite_name, time_day in data_tuple_list:
        ...
        if density_threshold != 0:
            ...
            for patch_idx, final_df_subset in enumerate(final_df_subset_list):
                final_df_subset_file_path = os.path.join(args.output_dir, f"middle_data/{satellite_name}_{time_day}_patch{patch_idx}-final_df.csv")
            ...
            final_max_index = max(final_df.dropna().index)
            final_min_index = min(final_df.dropna().index)
            final_df_subset = final_df[final_df.index.to_series().between(final_min_index, final_max_index)]
